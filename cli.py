...
❓ Need Help?
   ...
   BioAnalyzer <command> --help        Show command-specific help
    ...
    def check_image(self):
        ...
        try:
            subprocess.run(["docker", "image", "inspect", self.image_name], capture_output=True, check=True)
            return True
    ...
    def build_containers(self):
        ...
        try:
            ...
            result = subprocess.run(["docker", "build", "-t", self.image_name, "."], cwd=project_root, check=True)
            ...
            if frontend_dir.exists():
                ...
                subprocess.run(["docker", "build", "-t", "bioanalyzer-frontend", "."], cwd=frontend_dir, check=True)
    ...
    def _ensure_network(self):
        ...
        if not check_result.stdout.strip():
            ...
            subprocess.run(["docker", "network", "create", self.network_name], capture_output=True, check=False)

    def start_application(self):
        """Start the BioAnalyzer application using docker-compose."""
        ...
        try:
            ...
            try:
                ...
                print("   Please install docker-compose to use BioAnalyzer.")
                return False
        ...
        try:
            ...
            if self.check_backend_health():
                print("✅ Backend API is already available at http://localhost:8000")
                # Check if we should still start docker-compose to ensure all services are up
                ...
                if "bioanalyzer-backend" in status_result.stdout:
            ...
            if self._wait_for_backend_health(timeout=60, interval=2):
                ...
                logs_result = subprocess.run(
                    compose_cmd + ["logs", "bioanalyzer-backend", "--tail=30"],
                    cwd=project_root,
                    capture_output=True,
                    text=True,
                    check=False,
            ...
            print("📱 Web Interface: http://localhost:3000 (if frontend is configured)")
    ...
    def stop_application(self):
        """Stop the BioAnalyzer application using docker-compose."""
        ...
        try:
            ...
            try:
                ...
                print("⚠️  docker-compose not available, stopping containers manually...")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print("   You may need to manually stop containers")
   ...
   quit                      Exit interactive mode
    ...
    def handle_url_analysis(
        ...
        for index, url in enumerate(all_urls, 1):
            ...
            analysis_result = self._poll_url_analysis_job(job_id=job_id, poll_interval=poll_interval, timeout=timeout)
    ...
    def _start_url_analysis_job(self, url: str, embedding_model: str, llm_model: str) -> Optional[str]:
        """Submit a new URL analysis job."""
        try:
            payload = {"url": url, "embedding_model": embedding_model, "llm_model": llm_model}
            response = requests.post(self._build_api_url("/analyze-url"), json=payload, timeout=30)
            if response.status_code != 200:
    ...
    def _poll_url_analysis_job(self, job_id: str, poll_interval: int, timeout: int) -> Optional[Dict[str, Any]]:
   ...
   "What are the differences between metagenomics and 16S sequencing?"
                ...
                try:
                    ...
                    api_response = requests.post(
                        "http://localhost:8000/api/v1/qa", json={"question": user_input}, timeout=60
                    )
    ...
    async def ask_question(self, question: str) -> Optional[str]:
        ...
        try:
            ...
            api_response = requests.post("http://localhost:8000/api/v1/qa", json={"question": question}, timeout=60)
    ...
    def _fetch_single_paper(self, retriever, pmid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"pmid": pmid, "error": f"Retrieval failed: {str(e)}", "retrieval_timestamp": time.time()}
    ...
    def _create_standalone_retriever(self):
        ...
        class FallbackRetriever:
            def __init__(self):
                ...
                self.session.headers.update({"User-Agent": "BioAnalyzer/1.0 (contact: bioanalyzer@example.com)"})
...
Examples:
  ...
  BioAnalyzer stop                    # Stop application
    ...
    start_parser.add_argument("--interactive", "-i", action="store_true", help="Start in interactive mode")
    ...
    analyze_parser.add_argument(
        "--format", choices=["table", "json", "csv", "xml"], default="table", help="Output format"
    )
    ...
    analyze_url_parser.add_argument(
        "--embedding-model", default="gemini/text-embedding-004", help="Embedding model identifier"
    )
    analyze_url_parser.add_argument("--llm-model", default="gemini/gemini-2.0-flash", help="LLM model identifier")
    analyze_url_parser.add_argument(
        "--format", choices=["table", "json"], default="table", help="Output format for results"
    )
    analyze_url_parser.add_argument("--output", "-o", help="File to save results")
    analyze_url_parser.add_argument("--poll-interval", type=int, default=5, help="Seconds between status polls")
    analyze_url_parser.add_argument(
        "--timeout", type=int, default=300, help="Time in seconds before giving up on a job"
    )
    analyze_url_parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    ...
    qa_parser.add_argument(
        "question", nargs="?", help="Question to ask (optional, will start interactive mode if not provided)"
    )
    qa_parser.add_argument("--interactive", "-i", action="store_true", help="Start interactive Q&A mode")
    ...
    retrieve_parser.add_argument("--save", "-s", action="store_true", help="Save individual paper data to files")
    retrieve_parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    ...
    settings_save_parser.add_argument("--format", choices=["json", "yaml"], default="json", help="File format")
    ...
    settings_load_parser.add_argument("--apply", action="store_true", help="Apply loaded settings to environment")
    ...
    settings_preset_parser.add_argument(
        "name", choices=["fast", "balanced", "high_quality", "development", "production"], help="Preset name"
    )
    settings_preset_parser.add_argument("--save", "-s", action="store_true", help="Save preset to settings file")
    ...
    if args.command == "retrieve":
        ...
        asyncio.run(cli.retrieve_papers(unique_pmids, args.format, args.output, args.save))
        return
