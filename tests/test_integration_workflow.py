...
async def test_complete_workflow():
    ...
    for i, image_url in enumerate(scrape_result["images"][:3]):  # Limit to 3
        ...
        if parsed_media:
            image_descriptions.append(
                {"url": image_url, "description": f"Image {i+1} description"}  # Would use LLM here
            )
    ...
    chunks = await chunker.chunk_markdown(markdown=enhanced_md, doc_name="test_study", doc_key="test_001")
    ...
    vector_store = VectorStoreService(
        store_type="numpy", embedding_model="st-all-MiniLM-L6-v2"  # Fast local model for testing
    )
    ...
    results, scores = await vector_store.similarity_search(query="What is this study about?", k=3)
...
async def test_vector_store_with_ollama():
    ...
    try:
        vector_store = VectorStoreService(store_type="numpy", embedding_model="ollama/nomic-embed-text")
