...
def rag_service(temp_cache_dir, mock_llm_provider):
    ...
    with patch("app.services.contextual_summarization.LLMProviderManager") as mock_summary, patch(
...
class TestAdvancedRAGService:
    ...
    def test_initialization(self, temp_cache_dir):
        ...
        service = AdvancedRAGService(summary_provider="gemini", rerank_method="keyword", cache_dir=str(temp_cache_dir))
    ...
    async def test_retrieve_and_summarize(self, rag_service, sample_chunks):
        ...
        summaries, ranked_chunks = await rag_service.retrieve_and_summarize(chunks=sample_chunks, query=query, top_k=3)
    ...
    async def test_get_contextual_context(self, rag_service, sample_chunks):
        ...
        context = await rag_service.get_contextual_context(chunks=sample_chunks, query=query, top_k=3)
    ...
    async def test_get_contextual_context_fallback(self, rag_service, sample_chunks):
        ...
        query = "Test query"
        context = await rag_service.get_contextual_context(chunks=sample_chunks, query=query)
    ...
    async def test_different_rerank_methods(self, temp_cache_dir, sample_chunks, mock_llm_provider):
        ...
        for method in ["keyword", "llm", "hybrid"]:
            with patch("app.services.contextual_summarization.LLMProviderManager") as mock_summary, patch(
                ...
                service = AdvancedRAGService(rerank_method=method, cache_dir=str(temp_cache_dir))
                service.summarization_service.summary_llm = mock_llm_provider
                if method != "keyword":
                    service.reranker.llm_manager = mock_llm_provider

                context = await service.get_contextual_context(chunks=sample_chunks, query="Test query", top_k=2)
    ...
    async def test_get_summary_stats(self, rag_service, sample_chunks):
        ...
        query = "Test query"
        summaries, _ = await rag_service.retrieve_and_summarize(chunks=sample_chunks, query=query, top_k=3)
    ...
    async def test_top_k_filtering(self, rag_service, sample_chunks):
        ...
        for top_k in [1, 2, 3, 5]:
            summaries, ranked = await rag_service.retrieve_and_summarize(chunks=sample_chunks, query=query, top_k=top_k)
    ...
    async def test_empty_chunks(self, rag_service):
        ...
        summaries, ranked = await rag_service.retrieve_and_summarize(chunks=[], query="Test query")
    ...
    async def test_chunk_id_matching(self, rag_service, sample_chunks):
        ...
        ranked_ids = {getattr(rc.chunk, "name", f"chunk_{id(rc.chunk)}") for rc in ranked_chunks}
