...
def mock_accurate_llm():
    ...
    def chat_side_effect(messages, **kwargs):
        ...
        if "host species" in query.lower():
            return {
                "text": "The host species is Homo sapiens (human). All participants were human adults."
            }
            ...
            return {
                "text": "This chunk contains information about the study methodology and analysis."
            }
...
class TestRAGAccuracy:
    ...
    async def test_keyword_rerank_accuracy(self, accuracy_test_chunks):
        ...
        ranked = await reranker.rerank_chunks(chunks=accuracy_test_chunks, query=query)

        # Chunk with "host species" should be ranked highest
        top_chunk = ranked[0]
        assert (
            "host species" in top_chunk.chunk.text.lower()
            or "homo sapiens" in top_chunk.chunk.text.lower()
        )
    ...
    async def test_llm_rerank_accuracy(self, accuracy_test_chunks, mock_accurate_llm):
        ...
        with patch("app.services.chunk_reranking.LLMProviderManager") as mock_manager:
            ...
            ranked = await reranker.rerank_chunks(
                chunks=accuracy_test_chunks, query=query
            )
    ...
    @pytest.mark.asyncio
    async def test_hybrid_rerank_accuracy(
        self, accuracy_test_chunks, mock_accurate_llm
    ):
    ...
    @pytest.mark.asyncio
    async def test_summarization_accuracy(
        self, accuracy_test_chunks, mock_accurate_llm
    ):
        ...
        with patch(
            ...
            try:
                service = ContextualSummarizationService(
                    cache_dir=temp_dir, config=SummarizationConfig()
                )
                ...
                summary = await service.summarize_chunk(
                    chunk=relevant_chunk, query=query
                )

                # Summary should contain relevant information
                assert (
                    "homo sapiens" in summary.summary.lower()
                    or "human" in summary.summary.lower()
                )
    ...
    async def test_rag_pipeline_accuracy(self, accuracy_test_chunks, mock_accurate_llm):
        ...
        with patch(
            ...
            temp_dir = tempfile.mkdtemp()
            try:
                service = AdvancedRAGService(rerank_method="hybrid", cache_dir=temp_dir)
                ...
                context = await service.get_contextual_context(
                    chunks=accuracy_test_chunks, query=query, top_k=2
                )
    ...
    @pytest.mark.asyncio
    async def test_summary_quality_levels(
        self, accuracy_test_chunks, mock_accurate_llm
    ):
        ...
        with patch(
            ...
            try:
                for quality in ["fast", "balanced", "high"]:
                    service = ContextualSummarizationService(
                        cache_dir=temp_dir, config=SummarizationConfig(quality=quality)
                    )
                    service.summary_llm = mock_accurate_llm

                    summary = await service.summarize_chunk(
                        chunk=accuracy_test_chunks[0], query="What is the host species?"
                    )
