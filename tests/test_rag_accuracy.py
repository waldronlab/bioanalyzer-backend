...
class TestRAGAccuracy:
    ...
    async def test_keyword_rerank_accuracy(self, accuracy_test_chunks):
        ...
        top_chunk = ranked[0]
        assert "host species" in top_chunk.chunk.text.lower() or "homo sapiens" in top_chunk.chunk.text.lower()
    ...
    async def test_summarization_accuracy(self, accuracy_test_chunks, mock_accurate_llm):
        ...
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_manager:
            ...
            try:
                service = ContextualSummarizationService(cache_dir=temp_dir, config=SummarizationConfig())
                ...
                summary = await service.summarize_chunk(chunk=relevant_chunk, query=query)

                # Summary should contain relevant information
                assert "homo sapiens" in summary.summary.lower() or "human" in summary.summary.lower()
    ...
    async def test_rag_pipeline_accuracy(self, accuracy_test_chunks, mock_accurate_llm):
        ...
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_summary, patch(
            ...
            try:
                ...
                query = "What is the host species?"
                context = await service.get_contextual_context(chunks=accuracy_test_chunks, query=query, top_k=2)
