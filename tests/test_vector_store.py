...
def sample_texts():
    ...
    return [
        Text(text="This study examined the gut microbiome in human participants.", name="text_1"),
        Text(text="The host species was identified as Homo sapiens.", name="text_2"),
        Text(text="Samples were collected from the gastrointestinal tract.", name="text_3"),
    ]
...
class TestVectorStoreService:
    ...
    def test_initialization_numpy(self, mock_embedding_model):
        ...
        with patch("app.services.vector_store_service.embedding_model_factory") as mock_factory, patch(
            ...
            service = VectorStoreService(store_type="numpy", embedding_model="gemini/text-embedding-004")
    ...
    def test_initialization_qdrant(self, mock_embedding_model):
        ...
        with patch("app.services.vector_store_service.embedding_model_factory") as mock_factory, patch(
            ...
            service = VectorStoreService(
                store_type="qdrant", embedding_model="gemini/text-embedding-004", qdrant_path="/tmp/qdrant"
            )

            assert service.store_type == "qdrant"

    @pytest.mark.asyncio
    async def test_add_texts(self, sample_texts, mock_embedding_model, mock_vector_store):
    ...
    @pytest.mark.asyncio
    async def test_add_texts_with_existing_embeddings(self, sample_texts, mock_vector_store):
    ...
    async def test_similarity_search(self, mock_embedding_model, mock_vector_store):
        ...
        with patch("app.services.vector_store_service.embedding_model_factory") as mock_factory, patch(
            ...
            result_scores = [0.85]
            mock_vector_store.similarity_search = AsyncMock(return_value=(result_texts, result_scores))
    ...
    async def test_mmr_search(self, mock_embedding_model, mock_vector_store):
        ...
        with patch("app.services.vector_store_service.embedding_model_factory") as mock_factory, patch(
            ...
            result_texts = [Text(text="Result 1", name="r1")]
            result_scores = [0.85]
            mock_vector_store.mmr_search = AsyncMock(return_value=(result_texts, result_scores))
    ...
    def test_embedding_model_initialization(self):
        ...
        with patch("app.services.vector_store_service.embedding_model_factory") as mock_factory:
            mock_model = Mock()
