...
from app.services.contextual_summarization import ChunkSummary, ContextualSummarizationService, SummarizationConfig
...
def summarization_service(temp_cache_dir, mock_llm_provider):
    ...
    with patch("app.services.contextual_summarization.LLMProviderManager") as mock_manager:
        ...
        service = ContextualSummarizationService(
            ...
            config=SummarizationConfig(summary_length="medium", quality="balanced", max_key_points=3, use_cache=True),
        )
...
class TestSummarizationConfig:
    ...
    def test_custom_config(self):
        ...
        config = SummarizationConfig(
            summary_length="long", quality="high", max_key_points=10, temperature=0.5, use_cache=False
        )
...
class TestContextualSummarizationService:
    ...
    def test_initialization(self, temp_cache_dir):
        ...
        service = ContextualSummarizationService(cache_dir=str(temp_cache_dir), config=SummarizationConfig())
    ...
    def test_cache_key_generation(self, summarization_service):
        ...
        key3 = summarization_service._get_cache_key(chunk_text, "Different query", length)
        assert key1 != key3
    ...
    async def test_summarize_chunks(self, summarization_service, sample_chunks):
        ...
        summaries = await summarization_service.summarize_chunks(chunks=sample_chunks, query=query)
    ...
    @pytest.mark.asyncio
    async def test_summarize_chunks_with_cache(self, summarization_service, sample_chunks):
        """Test that caching works for summaries."""
        query = "What host species?"

        # First call - should generate summaries
        summaries1 = await summarization_service.summarize_chunks(chunks=sample_chunks, query=query)

        # Second call - should use cache
        summaries2 = await summarization_service.summarize_chunks(chunks=sample_chunks, query=query)
    ...
    @pytest.mark.asyncio
    async def test_summarize_chunks_cache_disabled(self, temp_cache_dir, mock_llm_provider):
        """Test summarization with cache disabled."""
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_manager:
            mock_manager.return_value = mock_llm_provider
            service = ContextualSummarizationService(
                cache_dir=str(temp_cache_dir), config=SummarizationConfig(use_cache=False)
            )
            service.summary_llm = mock_llm_provider

            chunks = [Text(text="Test chunk", name="chunk_1")]
            query = "Test query"
    ...
    @pytest.mark.asyncio
    async def test_different_summary_lengths(self, summarization_service, sample_chunks):
        """Test different summary length configurations."""
        query = "What is being studied?"

        for length in ["short", "medium", "long"]:
            service = ContextualSummarizationService(
                cache_dir=str(summarization_service.cache_dir), config=SummarizationConfig(summary_length=length)
            )
    ...
    async def test_summarize_without_llm(self, temp_cache_dir, sample_chunks):
        ...
        with patch("app.services.contextual_summarization.LITELLM_AVAILABLE", False):
            ...
            summaries = await service.summarize_chunks(chunks=sample_chunks, query="Test query")
    ...
    async def test_key_points_extraction(self, summarization_service, sample_chunks):
        ...
        mock_response = {"text": "Summary text. Key points: 1. Point one. 2. Point two. 3. Point three."}
        summarization_service.summary_llm.chat = AsyncMock(return_value=mock_response)

        summaries = await summarization_service.summarize_chunks(chunks=sample_chunks[:1], query=query)
    ...
    async def test_empty_chunks(self, summarization_service):
        ...
        summaries = await summarization_service.summarize_chunks(chunks=[], query="Test query")
        assert summaries == []
    ...
    async def test_llm_error_handling(self, summarization_service, sample_chunks):
        ...
        summarization_service.summary_llm.chat = AsyncMock(side_effect=Exception("LLM error"))

        # Should handle gracefully
        summaries = await summarization_service.summarize_chunks(chunks=sample_chunks, query="Test query")
