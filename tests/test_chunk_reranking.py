...
def sample_chunks():
    ...
    return [
        ...
        Text(text="Samples were collected from the gastrointestinal tract, specifically the colon.", name="chunk_3"),
        Text(
...
def mock_llm_provider():
    ...
    mock = Mock()
    mock.chat = AsyncMock(
        return_value={"text": "SCORE: 0.85 REASONING: This chunk contains highly relevant information about the query."}
    )
    return mock
...
class TestRankedChunk:
    ...
    def test_ranked_chunk_creation(self):
        ...
        ranked = RankedChunk(chunk=chunk, relevance_score=0.85, rank=1, reasoning="Highly relevant")
...
class TestChunkReRanker:
    ...
    def test_initialization_llm_method(self, mock_llm_provider):
        ...
        with patch("app.services.chunk_reranking.LLMProviderManager") as mock_manager:
            ...
            reranker = ChunkReRanker(rerank_method="llm", llm_provider="gemini", llm_model="gemini/gemini-2.0-flash")
    ...
    async def test_keyword_rerank(self, sample_chunks):
        ...
        host_chunk = next((r for r in ranked if "host species" in r.chunk.text.lower()), None)
    ...
    async def test_keyword_rerank_top_k(self, sample_chunks):
        ...
        ranked = await reranker.rerank_chunks(chunks=sample_chunks, query=query, top_k=3)
    ...
    async def test_keyword_rerank_exact_phrase(self, sample_chunks):
        ...
        exact_match = next((r for r in ranked if query.lower() in r.chunk.text.lower()), None)
        if exact_match:
    ...
    async def test_llm_rerank_with_reasoning(self, sample_chunks, mock_llm_provider):
        ...
        with patch("app.services.chunk_reranking.LLMProviderManager") as mock_manager:
            ...
            query = "Test query"
            ranked = await reranker.rerank_chunks(chunks=sample_chunks[:2], query=query)  # Just 2 for speed
    ...
    @pytest.mark.asyncio
    async def test_hybrid_rerank_large_chunk_list(self, sample_chunks, mock_llm_provider):
    ...
    async def test_score_parsing(self, sample_chunks, mock_llm_provider):
        ...
        with patch("app.services.chunk_reranking.LLMProviderManager") as mock_manager:
            ...
            for response_text in test_cases:
                ...
                ranked = await reranker.rerank_chunks(chunks=sample_chunks[:1], query="Test")
    ...
    async def test_calculate_keyword_score(self, sample_chunks):
        ...
        score1 = await reranker._calculate_keyword_score("This is about host species", "host species")
        assert 0.0 <= score1 <= 1.0

        # Test with no matches
        score2 = await reranker._calculate_keyword_score("This is about something else", "host species")
    ...
    def test_extract_reasoning(self):
        ...
        reasoning = reranker._extract_reasoning("SCORE: 0.85 REASONING: This is highly relevant")
