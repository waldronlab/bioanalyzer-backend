...
class TestEnhancedFieldValidator:
    ...
    def test_validate_field_with_text(self):
        ...
        test_cases = [
            (
                "host_species",
                {"value": "Human"},
                "This study included 50 human participants with IBD.",
            ),
            (
                "host_species",
                {"value": "Mouse"},
                "We used C57BL/6 mice in this experiment.",
            ),
            (
                "body_site",
                {"site": "Gut"},
                "Fecal samples were collected from the gut microbiome.",
            ),
            (
                "body_site",
                {"site": "Oral"},
                "Saliva samples were collected from the oral cavity.",
            ),
            (
                "sequencing_type",
                {"method": "16S rRNA"},
                "We performed 16S rRNA sequencing using V4 region.",
            ),
            (
                "sequencing_type",
                {"method": "Shotgun metagenomics"},
                "Whole genome shotgun metagenomics was performed.",
            ),
            (
                "condition",
                {"description": "IBD"},
            ...
            (
                "sample_size",
                {"size": "50"},
                "A total of n=50 participants were included in this study.",
            ),
        ]
    ...
    def test_validate_field_invalid_field_name(self):
        ...
        result = validator.validate_field(
            "invalid_field", {"value": "Test"}, "Some text"
        )
        assert "score" in result
    ...
    def test_check_pattern_match(self):
        ...
        result = validator._check_pattern_match(
            "host_species",
            "Human",
            "This study included human patients and mouse models.",
        )
    ...
    def test_get_validation_notes(self):
        validator = EnhancedFieldValidator()
        notes_present = validator._get_validation_notes(
            "host_species", "PRESENT", "Human", "Text"
        )
        notes_partial = validator._get_validation_notes(
            "host_species", "PARTIALLY_PRESENT", "Human", "Text"
        )
        notes_absent = validator._get_validation_notes(
            "host_species", "ABSENT", "", "Text"
        )
