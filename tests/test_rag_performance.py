...
class TestRAGPerformance:
        ...
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_manager:
            ...
            try:
                service = ContextualSummarizationService(
                    cache_dir=temp_dir,
                    config=SummarizationConfig(use_cache=False),  # Disable cache for consistent timing
                )
                ...
                start_time = time.time()
                summaries = await service.summarize_chunks(chunks=test_chunks, query=query)
                elapsed = time.time() - start_time
    ...
    async def test_rag_pipeline_performance(self, large_chunk_set, mock_fast_llm):
        ...
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_summary, patch(
            ...
            temp_dir = tempfile.mkdtemp()
            try:
                service = AdvancedRAGService(rerank_method="keyword", cache_dir=temp_dir)  # Use keyword for speed
                ...
                start_time = time.time()
                context = await service.get_contextual_context(chunks=test_chunks, query=query, top_k=5)
                elapsed = time.time() - start_time
    ...
    async def test_cache_performance_improvement(self, large_chunk_set, mock_fast_llm):
        ...
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_manager:
            ...
            temp_dir = tempfile.mkdtemp()
            try:
                service = ContextualSummarizationService(cache_dir=temp_dir, config=SummarizationConfig(use_cache=True))
                service.summary_llm = mock_fast_llm
    ...
    async def test_top_k_performance_impact(self, large_chunk_set, mock_fast_llm):
        ...
        with patch("app.services.contextual_summarization.LLMProviderManager") as mock_summary, patch(
            ...
            temp_dir = tempfile.mkdtemp()
            try:
                service = AdvancedRAGService(rerank_method="keyword", cache_dir=temp_dir)
                service.summarization_service.summary_llm = mock_fast_llm
                ...
                # Test with top_k=5
                start1 = time.time()
                context1 = await service.get_contextual_context(chunks=large_chunk_set[:20], query=query, top_k=5)
                elapsed1 = time.time() - start1

                # Test with top_k=10
                start2 = time.time()
                context2 = await service.get_contextual_context(chunks=large_chunk_set[:20], query=query, top_k=10)
                elapsed2 = time.time() - start2
