...
class TestCreateDefaultFieldStructure:
    ...
    def test_create_default_body_site(self):
        ...
        assert result["status"] == "ABSENT"
        assert result["value"] is None
        assert "gut" in result["suggestions"].lower() or "oral" in result["suggestions"].lower()
    ...
    def test_create_default_condition(self):
        ...
        assert result["status"] == "ABSENT"
        assert "disease" in result["suggestions"].lower() or "treatment" in result["suggestions"].lower()
    ...
    def test_create_default_sequencing_type(self):
        ...
        assert result["status"] == "ABSENT"
        assert "16s" in result["suggestions"].lower() or "sequencing" in result["suggestions"].lower()
    ...
    def test_create_default_taxa_level(self):
        ...
        assert result["status"] == "ABSENT"
        assert "phylum" in result["suggestions"].lower() or "genus" in result["suggestions"].lower()
    ...
    def test_create_default_sample_size(self):
        ...
        assert result["status"] == "ABSENT"
        assert "samples" in result["suggestions"].lower() or "participants" in result["suggestions"].lower()
...
class TestGenerateCurationSummary:
    ...
    def test_generate_summary_with_missing_fields(self):
        ...
        parsed_analysis = {
            "host_species": {"status": "ABSENT", "suggestions": "Look for mentions of human, mouse, rat"},
            "body_site": {"status": "ABSENT", "suggestions": "Look for mentions of gut, oral, skin"},
        }
    ...
    def test_generate_summary_includes_suggestions(self):
        ...
        parsed_analysis = {"host_species": {"status": "ABSENT", "suggestions": "Look for human, mouse, rat"}}
        missing = ["host_species"]
        summary = generate_curation_summary(parsed_analysis, missing)
        assert "suggestions" in summary.lower() or "human" in summary.lower() or "mouse" in summary.lower()
