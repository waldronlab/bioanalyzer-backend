...
class APIConfig(BaseModel):
    ...
    analysis_timeout: int = Field(
        default=45, ge=1, le=600, description="Total analysis timeout in seconds"
    )
    ...
    frontend_timeout: int = Field(
        ...
        ge=1,
        le=600,
        description="API client timeout in seconds (e.g. for long-running analysis)",
    )
    ncbi_timeout: int = Field(
        default=60, ge=1, le=300, description="NCBI API timeout in seconds"
    )
    ncbi_rate_limit_delay: float = Field(
        default=0.34, ge=0.0, le=10.0, description="NCBI rate limit delay in seconds"
    )
    max_concurrent_requests: int = Field(
        default=3, ge=1, le=20, description="Maximum concurrent requests"
    )
...
class LLMConfig(BaseModel):
    ...
    provider: Optional[str] = Field(
        default=None,
        description="LLM provider (openai, anthropic, gemini, ollama, llamafile)",
    )
    model: Optional[str] = Field(default=None, description="Specific model to use")
    api_key: Optional[str] = Field(
        default=None, description="API key (can be set via env var)"
    )
...
class RAGConfig(BaseModel):
    ...
    top_k_chunks: int = Field(
        default=10, ge=1, le=100, description="Number of top chunks to use"
    )
    rerank_method: RerankMethod = Field(
        default=RerankMethod.HYBRID, description="Re-ranking method"
    )
    summary_length: SummaryLength = Field(
        default=SummaryLength.MEDIUM, description="Summary length"
    )
    summary_quality: SummaryQuality = Field(
        default=SummaryQuality.BALANCED, description="Summary quality"
    )
    summary_provider: Optional[str] = Field(
        default=None, description="LLM provider for summarization"
    )
    summary_model: Optional[str] = Field(
        default=None, description="Model for summarization"
    )
    use_cache: bool = Field(default=True, description="Enable summary caching")
    max_summary_key_points: int = Field(
        default=5, ge=1, le=20, description="Maximum key points per summary"
    )
...
class CacheConfig(BaseModel):
    ...
    validity_hours: int = Field(
        default=24, ge=1, le=720, description="Cache validity in hours"
    )
    max_size: int = Field(
        default=1000, ge=1, le=100000, description="Maximum cache entries"
    )
...
class LoggingConfig(BaseModel):
    ...
    max_file_size: int = Field(
        default=10 * 1024 * 1024, ge=1024, description="Maximum log file size in bytes"
    )
    max_files: int = Field(
        default=5, ge=1, le=50, description="Maximum number of rotated log files"
    )
...
class RetrievalConfig(BaseModel):
    ...
    email: Optional[str] = Field(
        default=None, description="Email for NCBI API requests"
    )
    ncbi_api_url: str = Field(
        default="https://eutils.ncbi.nlm.nih.gov/entrez/eutils",
        description="NCBI API URL",
    )
...
class SecurityConfig(BaseModel):
    ...
    enable_request_id: bool = Field(
        default=True, description="Enable request ID tracking"
    )
...
class BioAnalyzerSettings(BaseModel):
    ...
    inherit_from: Optional[str] = Field(
        default=None, description="Path to settings file to inherit from"
    )
    ...
    def from_env(cls) -> "BioAnalyzerSettings":
        ...
        if os.getenv("NCBI_RATE_LIMIT_DELAY"):
            settings.api.ncbi_rate_limit_delay = float(
                os.getenv("NCBI_RATE_LIMIT_DELAY")
            )
        if os.getenv("MAX_CONCURRENT_REQUESTS"):
            settings.api.max_concurrent_requests = int(
        ...
        if os.getenv("RAG_SUMMARY_LENGTH"):
            settings.rag.summary_length = SummaryLength(
                os.getenv("RAG_SUMMARY_LENGTH").lower()
            )
        if os.getenv("RAG_SUMMARY_QUALITY"):
            settings.rag.summary_quality = SummaryQuality(
                os.getenv("RAG_SUMMARY_QUALITY").lower()
            )
        if os.getenv("RAG_RERANK_METHOD"):
            settings.rag.rerank_method = RerankMethod(
                os.getenv("RAG_RERANK_METHOD").lower()
            )
        if os.getenv("RAG_USE_SUMMARY_CACHE"):
            settings.rag.use_cache = os.getenv("RAG_USE_SUMMARY_CACHE").lower() in (
                "true",
                "1",
                "yes",
            )
        if os.getenv("RAG_MAX_SUMMARY_KEY_POINTS"):
            settings.rag.max_summary_key_points = int(
                os.getenv("RAG_MAX_SUMMARY_KEY_POINTS")
            )
        ...
        if os.getenv("ENABLE_RATE_LIMITING"):
            settings.rate_limit.enabled = os.getenv("ENABLE_RATE_LIMITING").lower() in (
                "true",
                "1",
                "yes",
            )
        if os.getenv("RATE_LIMIT_PER_MINUTE"):
            settings.rate_limit.requests_per_minute = int(
                os.getenv("RATE_LIMIT_PER_MINUTE")
            )

        # Retrieval settings
        if os.getenv("USE_FULLTEXT"):
            settings.retrieval.use_fulltext = os.getenv("USE_FULLTEXT").lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        if os.getenv("ENABLE_REQUEST_ID"):
            settings.security.enable_request_id = os.getenv(
                "ENABLE_REQUEST_ID"
            ).lower() in ("true", "1", "yes")
...
class SettingsManager:
    ...
    def save(
        ...
        if settings is None:
            raise ValueError(
                "No settings to save. Load settings first or provide settings parameter."
            )
    ...
    def migrate_settings(
        self, old_file: Path, new_file: Optional[Path] = None
    ) -> BioAnalyzerSettings:
