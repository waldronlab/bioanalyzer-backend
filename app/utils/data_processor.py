...
class BugSigConversationDataset(Dataset):
    ...
    def _prepare_conversation_input(
        ...
        if knowledge_indices:
            knowledge_texts = [
                self.knowledge_base.iloc[idx]["text"] for idx in knowledge_indices
            ]
            ...
            knowledge_padded = torch.zeros(
                len(knowledge_indices), self.max_length, dtype=torch.long
            )
            knowledge_mask = torch.zeros(
                len(knowledge_indices), self.max_length, dtype=torch.bool
            )
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        inputs = self._prepare_conversation_input(
            query=query, context=context, knowledge_indices=knowledge_indices
        )
        ...
        return {**inputs, "response_ids": response_padded}
