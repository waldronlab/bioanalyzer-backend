...
class MethodsScorer:
    ...
    def score_paper(self, text: str) -> MethodsScore:
        ...
        # Score each category
        experimental_score = self._score_category(
            text_lower, self.experimental_criteria
        )
        ...
        data_quality_score = self._score_category(
            text_lower, self.data_quality_criteria
        )
    ...
    def _score_category(
        self, text: str, criteria: Dict[str, List[str]]
    ...
    def get_improvement_suggestions(self, score: MethodsScore) -> List[str]:
        ...
        if score.sequencing_methods < 0.5:
            suggestions.append(
                "Include detailed sequencing protocols and quality control measures"
            )

        if score.analytical_methods < 0.5:
            suggestions.append(
                "Describe bioinformatics pipeline and analytical methods in detail"
            )

        if score.statistical_methods < 0.5:
            suggestions.append(
                "Specify statistical tests and multiple testing corrections"
            )

        if score.data_quality < 0.5:
            suggestions.append(
                "Provide data availability and code reproducibility information"
            )
