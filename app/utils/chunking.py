...
class ChunkingService:
    ...
    async def chunk_markdown(self, markdown: str, doc_name: str = "study", doc_key: Optional[str] = None) -> list[Text]:
        """Chunk markdown text into Text objects."""
        logger.info(f"Chunking markdown: {len(markdown)} chars (chunk_size={self.chunk_chars}, overlap={self.overlap})")

        doc = Doc(docname=doc_name, dockey=doc_key or doc_name, citation=f"{doc_name} - BioAnalyzer Study Analysis")
        ...
        chunks = chunk_text(
            parsed_text=parsed_text, doc=doc, chunk_chars=self.chunk_chars, overlap=self.overlap, use_tiktoken=True
        )
