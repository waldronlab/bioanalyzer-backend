...
class EnhancedFieldValidator:
    ...
    def __init__(self):
        self.field_patterns = {
            ...
            "sequencing_type": {
                ...
                "metatranscriptomics": [
                    r"metatranscriptomic",
                    r"metatranscriptomics",
                    r"rna-seq",
                    r"transcriptome",
                ],
                "other": [
    ...
    def validate_field(
        self, field_name: str, field_data: Dict, text: Optional[str] = ""
    ) -> Dict:
        """Validate a field based on extracted data and optional text content."""
        try:
            content_value = (
                field_data.get("value", "")
                or field_data.get("primary", "")
                or field_data.get("site", "")
                or field_data.get("description", "")
                or field_data.get("method", "")
                or field_data.get("level", "")
                or field_data.get("size", "")
                or ""
            )

            if not content_value or content_value.lower() in [
                "unknown",
                "not specified",
                "",
            ]:
            ...
            pattern_match = (
                self._check_pattern_match(field_name, content_value, text)
                if text
                else {"confidence": 0.5}
            )
    ...
    def _check_pattern_match(
        self, field_name: str, content_value: str, text: str
    ) -> Dict[str, float]:
    ...
    def _get_validation_notes(
        self, field_name: str, status: str, content_value: str, text: str
    ) -> str:
...
class FieldExtractionEnhancer:
    ...
    def enhance_extraction(self, extracted_data: Dict, full_text: str) -> Dict:
        ...
        for field_name in [
            ...
            if field_name in extracted_data:
                ...
                validation_result = self.validator.validate_field(
                    field_name, extracted_data[field_name], full_text
                )
                ...
                enhanced_data[field_name] = {
                    ...
                    "status": status,
                    "reason_if_missing": (
                        "Field not found in analysis" if status == "ABSENT" else ""
                    ),
                    "suggestions_for_curation": validation_result.get("notes", ""),
                }
            else:
                # Create default structure for missing field
                enhanced_data[field_name] = self._create_default_field_structure(
                    field_name
                )
        ...
        missing_fields = [
            field
            for field, data in enhanced_data.items()
            if data.get("status") != "PRESENT"
        ]

        enhanced_data["missing_fields"] = missing_fields
        enhanced_data["curation_preparation_summary"] = self._generate_curation_summary(
            missing_fields
        )
