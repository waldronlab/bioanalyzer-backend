...
async def ask_question(request: Dict[str, Any]):
    ...
    **Example:**
    ...
    try:
        ...
        unified_qa = get_unified_qa()
        if unified_qa is None:
            raise HTTPException(status_code=503, detail="QA service not available. Check GEMINI_API_KEY configuration.")
        ...
        if not answer:
            raise HTTPException(
                status_code=500, detail="No answer generated. Please check GEMINI_API_KEY and try again."
            )
