...
async def get_analysis_result(job_id: str) -> StudyAnalysisResult:
    ...
    if not job.result:
        raise HTTPException(
            status_code=500, detail=f"Job {job_id} completed but no result available"
        )
...
async def process_url_analysis(
    job_id: str, url: str, embedding_model: str, llm_model: str
):
    """
    ...
    try:
        ...
        if GEMINI_API_KEY:
            try:
                ...
                logger.warning(
                    f"Job {job_id}: Unable to initialize visual LLM: {safe_error}"
                )
            ...
            if parsed_media:
                ...
                if visual_llm:
                    generated_description = (
                        await image_processor.describe_image_with_llm(
        ...
        logger.info(
            f"Job {job_id}: Created enhanced markdown ({len(enhanced_md)} chars)"
        )
        ...
        chunks = await chunker.chunk_markdown(
            markdown=enhanced_md, doc_name=f"study_{job_id[:8]}", doc_key=job_id
        )

        vector_store = VectorStoreService(
            store_type="numpy", embedding_model=embedding_model
        )
        ...
        result = await orchestrator.analyze_study(
            chunks=chunks, study_id=job_id, source_url=url
        )
