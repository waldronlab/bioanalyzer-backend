...
async def analyze_url(request: AnalyzeURLRequest, background_tasks: BackgroundTasks) -> JobStatus:
    ...
    job_store[job_id] = JobStatus(job_id=job_id, status="pending", progress="Job queued")
...
async def get_analysis_result(job_id: str) -> StudyAnalysisResult:
    ...
    if not job.result:
        raise HTTPException(status_code=500, detail=f"Job {job_id} completed but no result available")

    return job.result


async def process_url_analysis(job_id: str, url: str, embedding_model: str, llm_model: str):
    """
    ...
    try:
        ...
        if GEMINI_API_KEY:
            try:
                visual_llm = UnifiedQA(use_gemini=True, gemini_api_key=GEMINI_API_KEY)
            ...
            if parsed_media:
                ...
                if visual_llm:
                    generated_description = await image_processor.describe_image_with_llm(
                        parsed_media=parsed_media, llm_service=visual_llm
                    )
                ...
                image_descriptions.append({"url": img_url, "description": description})
        ...
        chunks = await chunker.chunk_markdown(markdown=enhanced_md, doc_name=f"study_{job_id[:8]}", doc_key=job_id)

        vector_store = VectorStoreService(store_type="numpy", embedding_model=embedding_model)
        ...
        orchestrator = AgentOrchestrator(llm_model=llm_model, embedding_model=embedding_model)

        result = await orchestrator.analyze_study(chunks=chunks, study_id=job_id, source_url=url)

        logger.info(
            f"Job {job_id}: Extracted {len(result.experiments)} experiments, " f"{result.total_signatures} signatures"
        )
