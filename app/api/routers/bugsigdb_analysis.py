...
def _validate_pmid(pmid: str) -> str:
    ...
    if not pmid.isdigit():
        raise HTTPException(status_code=400, detail=f"Invalid PMID format: '{pmid}'. PMID must be numeric.")

    # Validate length (PMID is typically 8 digits, but can vary)
    if len(pmid) < 1 or len(pmid) > 20:
        raise HTTPException(status_code=400, detail=f"Invalid PMID length: '{pmid}'. PMID should be 1-20 digits.")
...
async def get_essential_fields():
    ...
    return {"essential_fields": ESSENTIAL_FIELDS_INFO, "status_values": STATUS_VALUES}
...
async def get_field_details(field_name: str):
    ...
    return {"field": field_details, "status_values": STATUS_VALUES}
