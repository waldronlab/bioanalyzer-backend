...
class PaperQAAgent:
    ...
    def __init__(
        ...
        if not PAPERQA_AVAILABLE:
            raise ImportError("Paper-QA is not installed. Please install it: " "pip install paper-qa")
    ...
    async def analyze_paper(self, paper_content: Dict[str, str]) -> Dict[str, Union[str, float, Dict[str, float]]]:
        ...
        try:
            ...
            # Add to Docs
            docs = Docs()
            await docs.aadd(path=str(temp_file), citation=title, title=title, settings=self.settings)
            ...
            return {"error": str(e), "confidence": 0.0, "key_findings": "", "curation_readiness": "UNKNOWN"}
    ...
    async def analyze_paper_enhanced(self, prompt: str) -> Dict[str, Union[str, float, List[str]]]:
        ...
        try:
            ...
            # Add to Docs
            docs = Docs()
            await docs.aadd(path=str(temp_file), citation="Paper Analysis", settings=self.settings)
            ...
            return {"analysis": answer_text, "confidence": 0.7, "key_findings": answer_text}

        except asyncio.TimeoutError:
            logger.error(f"Enhanced analysis timed out after {GEMINI_TIMEOUT}s")
            return {"error": "Analysis timed out", "key_findings": "{}", "confidence": 0.0}
        except Exception as e:
    ...
    async def ask_question(self, question: str, context: Optional[str] = None, pmid: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            return {"answer": answer_text, "confidence": 0.8, "pmid": pmid}
