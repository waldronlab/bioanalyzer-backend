...
class PaperQAAgent:
    ...
    def __init__(
        ...
        if not PAPERQA_AVAILABLE:
            raise ImportError(
                "Paper-QA is not installed. Please install it: " "pip install paper-qa"
            )

        self.api_key = api_key or os.getenv("GEMINI_API_KEY", "")
        self.model = model
        self.paper_directory = paper_directory or Path(
            tempfile.mkdtemp(prefix="paperqa_")
        )
        self.paper_directory.mkdir(parents=True, exist_ok=True)

        if not self.api_key:
            logger.warning(
                "No API key provided. Set GEMINI_API_KEY in your environment."
            )
    ...
    async def chat(self, prompt: str) -> Dict:
        ...
        try:
            ...
            answer_text = (
                ...
                if hasattr(response.session, "answer")
                else str(response)
            )
    ...
    async def analyze_paper(
        ...
        try:
            ...
            docs = Docs()
            await docs.aadd(
                path=str(temp_file), citation=title, title=title, settings=self.settings
            )
    ...
    async def analyze_paper_enhanced(
        ...
        try:
            ...
            docs = Docs()
            await docs.aadd(
                path=str(temp_file), citation="Paper Analysis", settings=self.settings
            )
