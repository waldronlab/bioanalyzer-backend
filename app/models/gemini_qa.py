...
class GeminiQA:
    ...
    def __init__(
        ...
        if not self.api_key:
            logger.warning(
                "No model API key provided. Set GEMINI_API_KEY in your environment."
            )
        genai.configure(api_key=self.api_key)

        try:
            available_models = [
                m.name for m in genai.list_models() if "gemini" in m.name.lower()
            ]
...
CRITICAL: If the paper contains ANY specific microbial taxa identification, abundance data, or microbial community analysis, it should be marked as READY FOR CURATION."""
            ...
            if self.results_dir:
                ...
                filename = (
                    self.results_dir
                    / f"model_analysis_{timestamp}_{paper_title[:50]}.txt"
                )
    ...
    async def analyze_paper_enhanced(
        ...
        try:
            ...
            if "quota" in error_msg.lower() or "quota exceeded" in error_msg.lower():
                error_detail = (
                    "Model API quota exceeded. Please check your API usage limits."
                )
                ...
                "authentication" in error_msg.lower() or "invalid" in error_msg.lower()
            ):
    ...
    def _validate_and_normalize_json(self, parsed_json: Dict) -> Dict:
        ...
        parsed_json["curation_preparation_summary"] = self._generate_curation_summary(
            parsed_json, missing_fields
        )
        ...
        try:
            ...
            for field_name, field_data in validated_json.items():
                ...
                if status == "PRESENT":
                    ...
                    if content_value and content_value.lower() not in [
                        "unknown",
                        "not specified",
                        "",
                    ]:
            ...
            if not confidence_scores:
                return 0.0
            weighted_scores = [
                score * 1.2 if score >= 0.8 else score for score in confidence_scores
            ]
    ...
    def _generate_curation_summary(
        self, parsed_json: Dict, missing_fields: List[str]
    ) -> str:
    ...
    async def chat(self, prompt: str) -> dict:
        try:
            ...
            return {
                "text": f"[Error: Request timed out after {GEMINI_TIMEOUT} seconds]",
                "confidence": 0.0,
            }
        except Exception as e:
