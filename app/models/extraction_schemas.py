...
class ExperimentMetadata(BaseModel):
    ...
    host_species: Optional[str] = Field(
        None,
        description="Host organism (e.g., Human, Mouse, Rat)"
    )
    body_site: Optional[str] = Field(
        None,
        description="Body site sampled (e.g., Gut, Oral, Skin)"
    )
    ...
    sequencing_type: Optional[str] = Field(
        None,
        description="Sequencing method (e.g., 16S, metagenomics)"
    )
    taxa_level: Optional[str] = Field(
        None,
        description="Taxonomic level analyzed (e.g., phylum, genus, species)"
    )
...
class MicrobialSignature(BaseModel):
    ...
    taxon_name: str = Field(
        description="Name of the microbial taxon"
    )
    ...
    statistical_significance: Optional[str] = Field(
        None,
        description="P-value or statistical test result"
    )
    comparison_group: Optional[str] = Field(
        None,
        description="What groups were compared"
    )
    evidence_text: str = Field(
        description="Text excerpt supporting this signature"
    )
...
class ExtractedExperiment(BaseModel):
    ...
    title: str = Field(
        description="Title or description of the experiment"
    )
    ...
    signatures: List[MicrobialSignature] = Field(
        ...
        description="Microbial signatures found"
    )
    methods_summary: Optional[str] = Field(
        None,
        description="Summary of experimental methods"
    )
    ...
    evidence_chunks: List[str] = Field(
        default_factory=list,
        description="Text chunks used as evidence"
    )
...
class StudyAnalysisResult(BaseModel):
    ...
    source_url: str = Field(
        description="Original URL of the study"
    )
    experiments: List[ExtractedExperiment] = Field(
        default_factory=list,
        description="Extracted experiments"
    )
    total_signatures: int = Field(
        default=0,
        description="Total number of signatures extracted"
    )
    curation_ready: bool = Field(
        default=False,
        description="Whether the study is ready for BugSigDB curation"
    )
