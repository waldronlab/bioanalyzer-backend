...
class ConverterService:
    ...
    async def create_enhanced_markdown(
        self, base_markdown: str, image_descriptions: list[dict], downloaded_files: list[dict], source_url: str
    ) -> str:
    ...
    async def _extract_file_content(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if suffix in [".txt", ".md", ".csv"]:
                parsed_text = await asyncio.to_thread(parse_text, file_path, html=False)
                return self._format_parsed_text(parsed_text)

            # HTML files
            elif suffix in [".html", ".htm"]:
                parsed_text = await asyncio.to_thread(parse_text, file_path, html=True)
                return self._format_parsed_text(parsed_text)
