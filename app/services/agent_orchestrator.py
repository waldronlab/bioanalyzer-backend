...
class AgentOrchestrator:
    ...
    async def analyze_study(self, chunks: List[Text], study_id: str, source_url: str) -> StudyAnalysisResult:
        ...
        if experiments:
            avg_confidence = sum(
                sum(sig.confidence for sig in exp.signatures) / max(len(exp.signatures), 1) for exp in experiments
            ) / len(experiments)
        else:
            avg_confidence = 0.0
    ...
    async def _extract_experiments(self, docs: Docs) -> List[ExtractedExperiment]:
        ...
        try:
            ...
            experiments = self._parse_experiments_from_response(response.session.answer, response.session.contexts)
    ...
    async def _extract_signatures(self, docs: Docs, experiment: ExtractedExperiment) -> List[MicrobialSignature]:
        ...
        try:
            ...
            signatures = self._parse_signatures_from_response(response.session.answer, response.session.contexts)
    ...
    def _parse_experiments_from_response(self, answer: str, contexts: list) -> List[ExtractedExperiment]:
        ...
        for line in lines:
            ...
            if any(marker in line.lower() for marker in ["experiment", "study", "cohort"]):
                if current_exp:
    ...
    def _parse_signatures_from_response(self, answer: str, contexts: list) -> List[MicrobialSignature]:
        ...
        for line in lines:
            ...
            if any(word in line.lower() for word in ["increased", "decreased", "enriched", "depleted"]):
    ...
    def _check_missing_fields(self, experiments: List[ExtractedExperiment]) -> List[str]:
