        ...
        if LITELLM_AVAILABLE and LLMProviderManager:
            try:
                self.summary_llm = LLMProviderManager(
                    provider=summary_llm_provider, model=summary_llm_model
                )
                logger.info(
...
5. Extract specific details that answer the query""",
    ...
    async def summarize_chunk(
        ...
        template = self.summary_templates.get(
            summary_length, self.summary_templates["medium"]
        )
        prompt = template.format(
            query=query, text=chunk_text[:3000]
        )  # Limit text length
        ...
        key_points = self._extract_key_points(
            chunk_text, query, max_points=self.config.max_key_points
        )

        # Calculate relevance score
        relevance_score = self._calculate_relevance_score(
            chunk_text, query, summary_text
        )
    ...
    def _extract_key_points(
        self, text: str, query: str, max_points: int = 5
    ) -> List[str]:
    ...
    def _calculate_relevance_score(self, text: str, query: str, summary: str) -> float:
        ...
        term_score = min(
            1.0, (text_matches + summary_matches * 2) / (len(query_terms) * 3)
        )
