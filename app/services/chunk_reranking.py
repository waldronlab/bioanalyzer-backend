...
class ChunkReRanker:
        ...
        self._metrics.update(
            {
                ...
                "avg_relevance_score": (
                    sum(rc.relevance_score for rc in scored_chunks) / len(scored_chunks)
                    if scored_chunks
                    else 0.0
                ),
                "max_relevance_score": max(
                    (rc.relevance_score for rc in scored_chunks), default=0.0
                ),
    ...
    async def _keyword_rerank(
        self, chunks: List[Text], query: str
    ) -> List[RankedChunk]:
        ...
        for chunk in chunks:
            ...
            scored_chunks.append(
                RankedChunk(
                    chunk=chunk, relevance_score=score, rank=0  # Will be assigned later
                )
            )
    ...
    async def _llm_rerank(self, chunks: List[Text], query: str) -> List[RankedChunk]:
        ...
        # Update metrics
        if chunk_times:
            self._metrics["avg_chunk_processing_time"] = sum(chunk_times) / len(
                chunk_times
            )
...
Format: SCORE: <number> REASONING: <explanation>"""
    ...
    async def _hybrid_rerank(self, chunks: List[Text], query: str) -> List[RankedChunk]:
        ...
        if (
            self.llm_manager and len(chunks) <= 20
        ):  # Only use LLM for reasonable number of chunks
            ...
            for keyword_ranked_chunk in keyword_ranked:
                if keyword_ranked_chunk.chunk in llm_scores:
                    ...
                    result.append(
                        RankedChunk(
                            ...
                            reasoning=next(
                                (
                                    rc.reasoning
                                    for rc in llm_ranked
                                    if rc.chunk == keyword_ranked_chunk.chunk
                                ),
                                None,
                            ),
                    ...
                    if self.use_10_scale:
                        keyword_ranked_chunk.relevance_score = (
                            keyword_ranked_chunk.relevance_score * 10.0
                        )
