...
class AdvancedRAGService:
    ...
    async def retrieve_and_summarize(
        ...
        summaries = await self.summarization_service.summarize_chunks(chunks=top_chunks, query=query)
        ...
        filtered_ranked = [
            rc for rc in ranked_chunks if getattr(rc.chunk, "name", f"chunk_{id(rc.chunk)}") in summary_chunk_ids
        ]
    ...
    async def get_contextual_context(
        ...
        summaries, ranked_chunks = await self.retrieve_and_summarize(
            chunks=chunks, query=query, top_k=top_k, evidence_k=evidence_k, max_sources=max_sources
        )
        ...
        if not context_parts:
            ...
            context_parts = [f"{chunk.text[:500]}" for chunk in chunks[: top_k or RAG_TOP_K_CHUNKS]]
    ...
    def get_summary_stats(self, summaries: List[ChunkSummary]) -> Dict:
        ...
        if not summaries:
            return {"num_summaries": 0, "avg_relevance": 0.0, "avg_confidence": 0.0, "total_key_points": 0}
