...
class WebScraperService:
    ...
    async def scrape_url_to_markdown(self, url: str) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Scraping complete: {len(markdown)} chars, " f"{len(image_urls)} images, {len(downloaded_files)} files"
            )
    ...
    async def _download_files(self, file_urls: list[str], base_url: str) -> list[dict]:
        ...
        async with aiohttp.ClientSession() as session:
            for url in file_urls:
                try:
                    downloaded_file = await self._download_single_file(session, url, base_url)
                    if downloaded_file:
    ...
    async def _download_single_file(self, session: aiohttp.ClientSession, url: str, base_url: str) -> Optional[dict]:
        ...
        try:
            async with session.get(url, timeout=self.timeout) as response:
                ...
                if content_length and int(content_length) > self.max_file_size:
                    logger.warning(f"Skipping {url}: file too large " f"({int(content_length) / 1024 / 1024:.2f} MB)")
                    return None
