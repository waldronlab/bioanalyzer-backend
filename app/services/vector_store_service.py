...
class VectorStoreService:
    ...
    def __init__(
        ...
        if self.store_type == "numpy":
            ...
            try:
                ...
                self.vector_store = QdrantVectorStore(client=client, collection_name=collection_name)
                logger.info(f"Using QdrantVectorStore: {collection_name}")
            except ImportError:
                logger.warning("qdrant-client not installed, falling back to NumpyVectorStore")
                self.vector_store = NumpyVectorStore()
        else:
    ...
    async def similarity_search(self, query: str, k: int = 5) -> tuple[list[Text], list[float]]:
        """
    ...
    async def mmr_search(self, query: str, k: int = 5, fetch_k: int = 20) -> tuple[list[Text], list[float]]:
        """
