...
class PubMedRetrievalService:
    ...
    async def retrieve_paper(
        self, pmid: str, save_to_file: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "error" in paper_data:
                logger.error(
                    f"Failed to retrieve paper data for PMID {pmid}: {paper_data['error']}"
                )
                return paper_data
    ...
    async def retrieve_multiple_papers(
        self, pmids: List[str], save_to_file: bool = False
    ) -> List[Dict[str, Any]]:
        """
