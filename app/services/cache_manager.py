...
class CacheManager:
    ...
    def store_analysis_result(
        self, pmid: str, analysis_data: Dict, metadata: Dict, source: str = "gemini", confidence: float = 0.0
    ) -> bool:
    ...
    async def store_analysis_result_async(
        ...
        return await loop.run_in_executor(
            None, self.store_analysis_result, pmid, analysis_data, metadata, source, confidence
        )
    ...
    def store_metadata(self, pmid: str, metadata: Dict, source: str = "pubmed") -> bool:
        ...
        try:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (pmid, json.dumps(metadata, ensure_ascii=False), datetime.now().isoformat(), source),
            )
    ...
    def get_metadata(self, pmid: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result:
                ...
                return {"metadata": json.loads(metadata), "timestamp": timestamp, "source": source, "cached": True}
    ...
    def get_fulltext(self, pmid: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result:
                ...
                return {"fulltext": fulltext, "timestamp": timestamp, "source": source, "cached": True}
        ...
        try:
            ...
            return {
                ...
                "curation_readiness_rate": (
                    ready_count / (ready_count + not_ready_count) if (ready_count + not_ready_count) > 0 else 0.0
                ),
            }
